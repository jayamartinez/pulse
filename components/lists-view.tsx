...
export function ListsView() {
  ...
  const active = dashboardLists[selected];
  ...
  return <div className="grid items-start gap-x-10 gap-y-10 lg:grid-cols-[300px_minmax(0,1fr)]">
    <section className="overflow-hidden border-t border-[var(--border)] lg:border-r lg:pr-8">
      <SectionHeader title="Your lists" detail={`${dashboardLists.length} total`} action={<button aria-label="Create list" className="grid h-7 w-7 place-items-center text-[var(--muted)] hover:text-white"><Plus className="h-3 w-3" /></button>} />
      <div className="divide-y divide-[var(--border)]">{dashboardLists.map((list, index) => <button key={list.name} onClick={() => setSelected(index)} className={`group flex h-[62px] w-full items-center rounded-md px-2 text-left transition-colors ${selected === index ? "bg-[var(--surface-2)]" : "hover:bg-[var(--surface)]"}`}>
    ...
    <section className="border-y border-[var(--border)]">
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-[var(--border)] py-3 text-[11px]"><span className="mr-1 text-[var(--muted-2)]">Included</span>{active.members.map(member => { const wallet = dashboardWallets.find(item => item.name === member); return <span key={member} className="text-[var(--muted)]">{wallet?.emoji && <span className="mr-1.5">{wallet.emoji}</span>}{member}</span>; })}{active.count > active.members.length && <span className="text-[var(--muted-2)]">+{active.count - active.members.length}</span>}</div>
    </section>
